...
UPDATE produtos SET prd_data_validade =  '2023-05-16'  WHERE prd_id = 1;
UPDATE produtos SET prd_data_validade = DATE_ADD(prd_data_fabricacao, INTERVAL 2 YEAR) WHERE prd_id = 2;
UPDATE produtos SET prd_data_validade =  '2023-03-20'  WHERE prd_id = 3;
UPDATE produtos SET prd_data_validade =  '2023-04-15'  WHERE prd_id = 4;
UPDATE produtos SET prd_data_validade = DATE_ADD(prd_data_fabricacao, INTERVAL 1 YEAR) WHERE prd_id = 5;

/* Criar ou substituir a visualização Produtos_Validade_Vencida */
CREATE OR REPLACE VIEW Produtos_Validade_Vencida AS
SELECT
    prd_nome,
    prd_data_validade
FROM produtos
...
/* Selecionar todos os dados da visualização Produtos_Validade_Vencida */
